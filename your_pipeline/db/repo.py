...
class Database:
    ...
    def upsert_vendors(self, rows: Iterable[Mapping]):
        ...
        stmt = stmt.on_conflict_do_update(index_elements=[Vendor.vendor_id],
    ...
    def upsert_brands(self, rows: Iterable[Mapping]):
        ...
        stmt = stmt.on_conflict_do_update(index_elements=[Brand.brand_id],
                                          set_={"vendor_id": stmt.excluded.vendor_id,
    ...
    def upsert_categories(self, rows: Iterable[Mapping]):
        ...
        stmt = stmt.on_conflict_do_update(index_elements=[Category.category_id],
                                          set_={"vendor_id": stmt.excluded.vendor_id,
                                                "name": stmt.excluded.name})
        with self.session() as s:
            s.execute(stmt)
    ...
    def upsert_shipping_tiers(self, rows: Iterable[Mapping]):
        ...
        stmt = stmt.on_conflict_do_update(index_elements=[ShippingTier.shipping_tier_id],
                                          set_={"vendor_id": stmt.excluded.vendor_id,
                                                "name": stmt.excluded.name,
    ...
    def upsert_repricings(self, rows: Iterable[Mapping]):
        ...
        stmt = stmt.on_conflict_do_update(index_elements=[RepricedProduct.sku],
                                          set_={"computed_price": stmt.excluded.computed_price,
                                                ...
                                                "computed_at": None})  # let trigger refresh timestamp
        with self.session() as s:
            s.execute(stmt)
